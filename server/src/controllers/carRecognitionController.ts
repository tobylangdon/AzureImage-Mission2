...
// I would have all the interfaces in an external file to make it easier to import into other files when using typescript
...
const carTypesToTags: CarTypes[] = [
  ...
  {
    type: "SUV",
    alternatives: ["compact sport utility vehicle", "sport utility vehicle"],
  },
  ...
  {
    type: "Super Car",
    alternatives: ["lamborghini", "maserati", "bugatti", "ferrari", "supercar"],
  },
];

const callAzureApiWithImageData = async (
  base64Image: string
...
const callAzureApiURL = async (
  url: string
): Promise<AzureResponse | boolean> => {
  // You might want to store your key in an environment variable or a config file
...
const mostLikelyType = (tags: AzureTag[]): TypeAndColour => {
  ...
  //   maybe name it carTypeConfidence instead of carTypeC"O"nfidence
  ...
  tags.forEach((tag) => {
    for (const type of carTypesToTags) {
      ...
      if (
        (type.alternatives.includes(tag.name) && !carTypeName) ||
        tag.name === "minivan" ||
        tag.name === "mini mpv" ||
        tag.name === "hatchback"
      ) {
      ...
      if (tag.confidence > 0.85 && type.alternatives.includes(tag.name)) {
        carTypes.push(tag);
        !carProbability[type.type]
          ? (carProbability[type.type] = 1)
          : (carProbability[type.type] += 1);
      }
    }
...
export const checkImage = async (
  req: Request,
  res: Response<CheckImageResponse | ErrorImageResponse>
): Promise<void> => {
  try {
    ...
    if (azureRes) {
      ...
      res.status(400).send({
        message: "The image could not be used. Please try another image.",
      });
    }
  } catch {
    res.status(500).send({
      message: "An internal server error occured. Please try a different image",
    });
  }
};
